...
import { Crown } from "./items/Fall_guys_crown";

export default function Player() {
  const hasCrown = useGame((state) => state.hasCrown);
  ...
  const [playerPosition, deathCount, levelIndex, death] = useGame((state) => [
...
//   useEffect(() => {
//     if (playerPosition) {
//       //   ref.current.setTranslation(playerPosition);
//       if (localStorage.getItem("playerPosition")) {
//         const position = JSON.parse(localStorage.getItem("playerPosition"));
//         console.log("player position2", position);
//         // ref.current.setTranslation({x:position.x, y:position.y+2,z: position.z})
//         // ref.current.setLinvel({ x: 0, y: 0, z: 0 });
//         // ref.current.setAngvel({ x: 0, y: 0, z: 0 });
//       }
//     }
  ...
  return (
    ...
    >
      <Ecctrl
        ...
        name="player"
      >
        <Crown scale={0.2} position={[0, 1, 0]} visible={hasCrown} />
